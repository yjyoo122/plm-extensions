...
rem  PORT
rem ---------------------------------------------------------------------------------------------------
rem  The server derives its port and protocol from the redirect URI in environment.js, unless the PORT
rem  environment variable overrides the port. The same order is used here so the browser opens on the
rem  right address even after the setup wizard has changed it.

set "PLMX_SCHEME=http"
set "PLMX_PORT=8080"

findstr /b /c:"exports.redirectUri" "!PLMX_HOME!environment.js" 2>nul | findstr /c:"https://" >nul 2>&1
if not errorlevel 1 set "PLMX_SCHEME=https"

for /f "tokens=3 delims=:" %%p in ('findstr /b /c:"exports.redirectUri" "!PLMX_HOME!environment.js" 2^>nul') do (
